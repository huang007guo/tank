...
function tankobj(x,y,rotate,speed,ismy){
	...
	if(typeof tankobj._isfun == 'undefined')
	{
		tankobj.prototype.uprotate = function (rotate){
			...
			if((rotate-this.rotate!=Math.PI/2&&rotate-this.rotate!=-Math.PI/2&&rotate-this.rotate!=(3*Math.PI/2)&&rotate-this.rotate!=-(3*Math.PI/2))&&!(ismy))
			{
				return false;
			}
			this.rotate = rotate;
		...
		tankobj.prototype.show = function (){
			...
			if(this.ismy)
			{
				...
				cxt.drawImage(flag,-flag.width/2,-flag.height/2+15);
